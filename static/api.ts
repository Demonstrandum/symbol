declare const __SYMBOL_ARTIFACT__: string;
declare const __SYMBOL_API_VERSION__: string;
declare const __SYMBOL_API_REVISION__: number;
declare const __SYMBOL_SOURCE_HASH__: string;
declare const __SYMBOL_GENERATOR_VERSION__: string;
declare const __SYMBOL_BUILD_COMMIT__: string;
declare const __SYMBOL_BUILD_DIRTY__: boolean;
declare const __SYMBOL_CONTRACT_FIXTURE__: EmbeddedContractFixture;
...
export const metadata: Readonly<SymbolApiMetadata> = Object.freeze({
    artifact: __SYMBOL_ARTIFACT__,
    apiVersion: __SYMBOL_API_VERSION__,
    absoluteRevision: __SYMBOL_API_REVISION__,
    sourceHash: __SYMBOL_SOURCE_HASH__,
    generatorVersion: __SYMBOL_GENERATOR_VERSION__,
    commit: __SYMBOL_BUILD_COMMIT__,
    dirty: __SYMBOL_BUILD_DIRTY__,
});
...
const CONTRACT_FIXTURE: EmbeddedContractFixture = __SYMBOL_CONTRACT_FIXTURE__;
...
export class MediaType {
    ...
    constructor(
        type: string,
        subtype: string,
        parameters: Iterable<MediaParameter> = [],
    ) {
...
export function allocatedFileName(hash: string, parts: GeneratedNameParts = {}): string {
    ...
    const extension = parts.extension === undefined || parts.extension === ""
        ? ""
...
export interface EndpointErrorMap {
    ...
    readonly "site move": ValidationError | UnauthorizedError | NotFoundError | ConflictError | ServerError;
...
export class SymbolClient {
    constructor(options: SymbolClientOptions = {}) {
        ...
        const fetchImplementation = options.fetch === undefined
            ? globalThis.fetch
            : options.fetch;
        if (typeof fetchImplementation !== "function") {
            throw new TypeError("SymbolClient requires a fetch implementation");
        }
        const retryPolicy = options.retryPolicy === undefined
...
export class SiteClient {
    ...
    copy(
        ...
        const destination = typeof destinationOrOptions === "string"
            ? destinationOrOptions
            : undefined;
        const options = typeof destinationOrOptions === "string"
            ? explicitOptions
            : destinationOrOptions ?? {};
        const idempotencyKey = destination === undefined
        ...
        const merged = destination === undefined
    ...
    setExpiry(
        ...
        const segments = path === undefined
            ? [this.name]
            : [this.name, ...pathSegments(normalizedPath(path))];
...
export class FolderClient {
    ...
    create(body: BodyInit, options: CreateFileOptions = {}): Operation<AllocationReceipt> {
        const configured = options.mediaType === undefined
    ...
    private allocate(
        ...
        const extension = naming.extension === undefined
            ? inferredFormat.preferredExtension
            : naming.extension === ""
                ? null
...
export class FileClient {
    ...
    patch(
        ...
        return new Operation({
            ...
            executor: async (signal, reportStatus) => {
                if (prepared === null) {
                    prepared = prepareSplices(descriptors, options.format === undefined ? "auto" : options.format);
                }
                const splice = await prepared;
                const headers = new Headers(splice.headers);
                headers.set("If-Content-Match", options.baseHash);
                const response = await executeRequest(this.client, {
                    ...
                    retrySafe: true,
                    successStatuses: [200],
                    decode: async (value) => decodeSpliceReceipt(
                        value,
                        idempotencyKey,
                        descriptors.length,
                        expectedSizeDelta,
...
function customAllocation(
    ...
    const cancel = async (): Promise<void> => {
        ...
        const headers = new Headers({
            ...
            "Allocation-Token": state.proposal.allocationToken,
        });
        const plan: RequestPlan<Response> = {
            ...
            successStatuses: [200],
            decode: async (response) => {
                await decodeCancellationReceipt(
                    response,
                    cancelKey,
                    state.proposal!.allocationToken,
                );
                return response;
            },
            dispose: null,
    ...
    return new Operation({
        ...
        executor: async (signal, reportStatus) => {
            if (state.proposal === null) {
                ...
                state.proposal = await executeRequest(client, {
                    endpoint: "allocated file",
                    path: allocationPath(site, folder),
                    method: "POST",
                    options,
            ...
            if (state.chosenName === null) {
                state.proposedResource = new AllocationProposal(state.proposal.proposal, cancel);
                try {
            ...
            const receipt = await executeRequest(client, {
                endpoint: "allocated file",
                path: allocationPath(site, folder),
                method: "POST",
                options,
                ...
                retrySafe: true,
                successStatuses: [200, 201],
                decode: async (response) => decodeAllocationReceipt(
                    response,
                    finalizeKey,
                    logicalKey,
...
async function executeRequest<T>(
    ...
    try {
        ...
        const preserved = response.bodyUsed ? new Response(null, {
...
function hostedResponseOperation(
    ...
    const successes = endpoint === "site index"
        ? [200, 206, 304, 307]
        : endpoint === "site file"
            ? [200, 206, 304, 307]
            : [200, 206, 304];
...
function normalizedPath(path: string, allowEmpty = false): string {
    ...
    const components = path.replace(/\\/g, "/").split("/").filter((part) => part !== "");
    if (
...
function validateBasename(value: string): string {
    if (
        ...
        || ["FILES", "HASH", "UNDO", "EXPIRES", "symbol.toml", ".symbol-token", ".symbol-claim"].includes(value)
    ) {
...
function normalizedParameterValue(name: string, value: string): string {
    if (typeof value !== "string" || Array.from(value).some((character) => !isQuotedCharacter(character))) {
...
function retryDelay(error: unknown, policy: RetryPolicy, retryIndex: number): number {
    ...
    if (
        ...
        && error instanceof SymbolApiError
    ) {
...
function mergedHeaders(input: HeadersInit | undefined, protocol: Headers): Headers {
    ...
    protocol.forEach((value, name) => headers.set(name, value));
    return headers;
}
...
function mergedMutationOptions(
    base: RequestOptions,
...
function formatForMediaType(value: string): ContentFormat {
    ...
    return formats.find((format) => format.mediaType.essence === mediaType.essence)
...
async function prepareSplices(
    ...
    const encoded = descriptors
        .map((change) => `offset=${change.offset}; delete=${change.deleteBytes}; insert=${change.insertBytes}`)
        .join(",");
    const canUseHeaders = descriptors.length <= CONTRACT_FIXTURE.splice.header_max_descriptors
...
async function decodeCachedDirectoryListing(
    response: Response,
...
async function decodeFileInventory(response: Response): Promise<FileInventory> {
    ...
    const aliases = array(value.aliases, "file inventory.aliases")
        .map((entry, index) => decodeAliasInventory(entry, `file inventory.aliases[${index}]`));
...
async function decodeUndoStack(response: Response): Promise<UndoStack> {
    ...
    const kinds: readonly string[] = [
        "put", "delete_path", "delete_site", "copy", "move", "expiry", "expire_sweep",
        "put_file", "allocate", "replace", "splice", "alias",
    ];
...
async function decodeExpirySiteReport(response: Response): Promise<ExpirySiteReport> {
    ...
    const entries = array(value.entries, "expiry inventory.entries")
        .map((entry, index) => decodeExpiryValue(entry, `expiry inventory.entries[${index}]`));
    return Object.freeze({
...
function decodeExpiryValue(value: unknown, label: string): ExpiryReport {
    ...
    const target: ExpiryTarget = targetKind === "site"
        ? Object.freeze({
    ...
    const limitedBy = report.limited_by === null
        ? null
        : (() => {
    ...
    return Object.freeze({
        ...
        ownPolicy: report.own_policy === null
            ? null
...
function decodeExpiryPolicy(value: unknown, label: string): ExpiryPolicy {
    const policy = exactRecord(
        value,
        [
            "mode", "min_age_seconds", "max_age_seconds", "max_size_bytes",
            "power", "retention_seconds", "expires_at",
        ],
        label,
    );
...
async function decodeSitePut(
    response: Response,
): Promise<SitePutReceipt> {
...
async function decodeFilePut(
    response: Response,
): Promise<FilePutReceipt> {
    const preserved = response.clone();
    requireMediaType(response, "text/plain");
    const body = (await response.text()).trim();
...
async function decodeAliasBatchReceipt(
    ...
    const aliases = array(value.aliases, "alias batch receipt.aliases")
        .map((entry, index) => decodeAliasInventory(entry, `alias batch receipt.aliases[${index}]`));
    return Object.freeze({
        ...mutation,
        status: response.status as 200 | 201,
        aliases: Object.freeze(aliases),
...
async function decodeAllocationReceipt(
    ...
    const mode = enumValue(namingValue.mode, ["generated", "custom"] as const, "allocation.naming.mode");
    const naming = mode === "custom"
        ? (() => {
    ...
    if (
        ...
        || (expectedProposal !== null
            && (hash !== expectedProposal.hash || unsigned(value.size, "allocation.size") !== expectedProposal.size))
    ) {
...
async function decodeFileReplaceReceipt(
    ...
    if (
        (outcome === "unchanged") === mutation.changed
        || (outcome === "relocated") !== relocated
    ) {
...
async function decodeProposalReceipt(
    ...
    return Object.freeze({
        ...
        proposal: Object.freeze({
            ...
            inferredExtension: nullableString(
                proposal.inferred_extension,
                "proposal.inferred_extension",
            ),
        }),
        idempotencyKey,
    ...
    switch (status) {
        case 400:
            return new ValidationError(
                "ValidationError", operation, status, body, idempotencyKey, response,
            );
        case 401: {
        ...
        case 403:
            return new ForbiddenError(
                "ForbiddenError", operation, status, body, idempotencyKey, response,
            );
        case 404:
            return new NotFoundError(
                "NotFoundError", operation, status, body, idempotencyKey, response,
            );
        case 405:
            return new MethodNotAllowedError(operation, body, idempotencyKey, response);
        case 409:
            return new ConflictError(
                "ConflictError", operation, status, body, idempotencyKey, response,
            );
        case 412: {
        ...
        case 413:
            return new PayloadTooLargeError(
                "PayloadTooLargeError", operation, status, body, idempotencyKey, response,
            );
        case 416:
            return new RangeNotSatisfiableError(operation, body, idempotencyKey, response);
        case 500:
            return new ServerError(
                "ServerError", operation, status, body, idempotencyKey, response,
            );
        default:
...
function dateValue(value: unknown, label: string): Date {
    ...
    if (!/^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-5][0-9](?:\.[0-9]+)?(?:Z|[+-][0-9]{2}:[0-9]{2})$/.test(text)) {
...
function nullableAliasKind(
    value: unknown,
    label: string,
): "file" | "directory" | null {
    return value === null
        ? null
        : enumValue(value, ["file", "directory"] as const, label);
}
